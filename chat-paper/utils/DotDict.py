...
class DotDict(dict):
    ...
    # def iter_hasattr func
    @staticmethod
    def iter_hasattr(obj, key):
        """
        Check whether have the corresponding key.
        :param obj: The `dict` object to get its val iteratively.
        :param key: The query key, list or str.
        :return flag: Indicate whether have the corresponding key.
        """
        try:
            DotDict.iter_getattr(obj, key); return True
        except Exception:
            return False
    ...
    def dotdictlst2dotdict(dotdictlst):
        ...
        for item in dotdictlst:
            ...
            if not dotdict.keys():
                for key_i in keys: DotDict.iter_setattr(dotdict, key_i, [DotDict.iter_getattr(item, key_i),])
            # For all next items, add the components to the existing list.
            else:
                for key_i in keys: DotDict.iter_getattr(dotdict, key_i).append(DotDict.iter_getattr(item, key_i))
    ...
    def dotdict2dotdictlst(dotdict):
        ...
        for key_i in keys:
            if not n_iters:
                n_iters = len(DotDict.iter_getattr(dotdict, key_i))
            else:
                assert n_iters == len(DotDict.iter_getattr(dotdict, key_i))
        ...
        for iter_idx in range(n_iters):
            ...
            for key_i in keys: DotDict.iter_setattr(dotdict_i, key_i, DotDict.iter_getattr(dotdict, key_i)[iter_idx])
    ...
    def dotdict2numpydict(dotdict):
        ...
        for key_i in keys:
            try:
                DotDict.iter_setattr(numpydict, key_i, np.array(DotDict.iter_getattr(dotdict, key_i)))
            except Exception:
                DotDict.iter_setattr(numpydict, key_i, DotDict.iter_getattr(dotdict, key_i))
